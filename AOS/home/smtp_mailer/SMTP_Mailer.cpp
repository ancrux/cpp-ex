...
int
SMTP_Mailer::connect(const char* ip_addr, unsigned short port, long connect_timeout, long socket_timeout, SMTP_Mailer_Domain_Queue* queue)
{
	std::string key(ip_addr);
	SMTP_Mailer_Connection* conn = this->create_connection(key.c_str(), queue);
	if ( conn )
	{
		conn->connect(ip_addr, port, connect_timeout, socket_timeout); // connect() must fire after create_connection()
		return 0;
	}
