...
static char *get_os_name(const OSVERSIONINFOEX osinfo)
{
	...
	if (osinfo.wServicePackMajor != 0) {
		if(osinfo.wServicePackMinor == 0) {
			if(_snprintf(tmp, sizeof(tmp), " SP%d", osinfo.wServicePackMajor) != -1) {
				r = sg_concat_string(&name, tmp);
			}
		} else {
			if(_snprintf(tmp, sizeof(tmp), " SP%d.%d", osinfo.wServicePackMajor,
...
sg_host_info *sg_get_host_info()
{
	...
	if (!runonce) { 
		...
		if(GetComputerNameA(name, &nameln) == 0) {
			free(name);
		...
		if(_snprintf(tmp, sizeof(tmp), "%ld", osinfo.dwBuildNumber) == -1) {
			free(tmp);
			return NULL;
		}
		...
		if(_snprintf(tmp, sizeof(tmp), "%ld.%ld", osinfo.dwMajorVersion,
