...
int add_counter(const char *fullCounterPath, HCOUNTER *phCounter)
{
	...
	//pdh_status = PdhAddCounterA(h_query, "\\Processor(_Total)\\% Processor Time", 0, phCounter);
	pdh_status = PdhAddCounterA(h_query, fullCounterPath, 0, phCounter);
...
static char *get_instances(const char *object)
{
	...
	pdh_status = PdhEnumObjectItemsA(NULL, NULL, object, NULL,
	...
	if(pdh_status == PDH_MORE_DATA || pdh_status == ERROR_SUCCESS) {
		...
		if (instancelistbuf != NULL && counterlistbuf != NULL) {
			pdh_status = PdhEnumObjectItemsA(NULL, NULL, object,
...
static int add_all_monitors()
{
	...
	int rc;
	rc = add_counter(PDH_USER, &(current_han[SG_WIN32_PROC_USER]));
	rc = add_counter(PDH_PRIV, &(current_han[SG_WIN32_PROC_PRIV]));
	rc = add_counter(PDH_IDLE, &(current_han[SG_WIN32_PROC_IDLE]));
	rc = add_counter(PDH_INTER, &(current_han[SG_WIN32_PROC_INT]));
	rc = add_counter(PDH_MEM_CACHE, &(current_han[SG_WIN32_MEM_CACHE]));
	rc = add_counter(PDH_UPTIME, &(current_han[SG_WIN32_UPTIME]));
	rc = add_counter(PDH_PAGEIN, &(current_han[SG_WIN32_PAGEIN]));
	rc = add_counter(PDH_PAGEOUT, &(current_han[SG_WIN32_PAGEOUT]));
	...
	if (diskio_names != NULL) {
		...
		for (i = 0; i < diskio_no; i++) {
			_snprintf(tmp, sizeof(tmp), PDH_DISKIOREAD, diskio_names[i]);
			add_counter(tmp, &diskio_rhan[i]);

			_snprintf(tmp, sizeof(tmp), PDH_DISKIOWRITE, diskio_names[i]);
