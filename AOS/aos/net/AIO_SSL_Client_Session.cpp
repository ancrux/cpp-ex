#include "AIO_SSL_Client_Session.h"

AIO_SSL_Client_Session::AIO_SSL_Client_Session(ACE_SSL_Context* context, int start_ssl)
:
AIO_Session(),
ssl_stream_(TRB_SSL_Asynch_Stream::ST_CLIENT, context),
is_cancelling_(0),
is_safe_to_delete_(0),
ssl_(start_ssl)
{
	//ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%t) ctor(%@):AIO_SSL_Client_Session\n"), this)); //@
}

AIO_SSL_Client_Session::~AIO_SSL_Client_Session()
{
	//ACE_DEBUG((LM_DEBUG, ACE_TEXT("(%t) dtor(%@):AIO_SSL_Client_Session\n"), this)); //@
}

void
AIO_SSL_Client_Session::cancel()
{
	if ( ssl_ )
	{
...
int
AIO_SSL_Client_Session::is_safe_to_delete() const
{
	if ( ssl_ )
	{
...
void
AIO_SSL_Client_Session::open(ACE_HANDLE handle, ACE_Message_Block& mb)
{
	if ( ssl_ )
	{
		{
...
void
AIO_SSL_Client_Session::start_ssl()
{
	if ( !ssl_ )
	{
		{
...
int
AIO_SSL_Client_Session::write(ACE_Message_Block& mb)
{
	if ( ssl_ )
	{
		if ( is_cancelling_ ) return -1;
...
void
AIO_SSL_Client_Session::handle_write_stream(const TRB_Asynch_Write_Stream::Result& result)
{
	if ( ssl_ )
	{
		{
...
void
AIO_SSL_Client_Session::handle_wakeup()
{
	if ( ssl_ )
	{
		{
